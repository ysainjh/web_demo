...
switch ($step) {
    ...
    case '4':
        if (intval($_GET['install'])) {
			...
			$config['SITE_INFO']['LANG_SWITCH_ON'] = 1; //描述
			$config['SITE_INFO']['version'] = 'demo'; //描述
			
			$config['SITE_INFO']['url'] = trim($_POST['siteurl']); //描述
			$config['LISTNUM']['DEFAULT_THEME'] = 'default'; //模版
			$config['LISTNUM']['prolist'] = 12; //产品
			$config['LISTNUM']['newslist'] = 12; //新闻
			$config['LISTNUM']['noIndColor'] = '#4f6128'; //主题颜色
			
			$config['TOKEN']['false_static'] = 1; //
			$config['TOKEN']['mess_on'] = 1; //
